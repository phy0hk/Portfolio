...
const AppFrameBar = ({ processInfo }: AppFrameBarProps) => {
  ...
  const startPos = useRef<PositionType>({ x: 0, y: 0, z: 999 });
  const dispatch = useDispatch();
  const [workderResult, setWorkerResult] = useState(null);
  const handleMouseDown = (e: React.MouseEvent) => {
    startPos.current = { x: e.clientX, y: e.clientY, z: 999 };
  ...
  const handleMouseMove = (e: MouseEvent) => {
    ...
    const newPos: PositionType = { x: 0, y: 0, z: 999 };
